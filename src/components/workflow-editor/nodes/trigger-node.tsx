...
import type { TriggerStepConfig } from "@/lib/engine/types";

function TriggerNode({
  data,
  selected,
}: NodeProps<Node<WorkflowNodeData>>) {
  const config = data.config as TriggerStepConfig;
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-muted-foreground/60 mt-1">{config.triggerType}</p>
