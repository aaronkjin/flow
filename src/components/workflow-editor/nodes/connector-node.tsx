...
import type { ConnectorStepConfig } from "@/lib/engine/types";

function ConnectorNode({
  data,
  selected,
}: NodeProps<Node<WorkflowNodeData>>) {
  const config = data.config as ConnectorStepConfig;

  return (
    <div
      className={cn(
        "rounded-lg border border-violet-400/20 bg-card shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:shadow-[0_2px_8px_rgba(0,0,0,0.06)] transition-shadow min-w-[180px] p-3",
    ...
    >
      ...
      <p className="text-xs text-muted-foreground/60 mt-1">
        {config.connectorType} / {config.action}
      </p>
